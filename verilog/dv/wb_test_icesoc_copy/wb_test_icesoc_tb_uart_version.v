...
module wb_test_icesoc_tb;
	...
	initial begin
		...
		*/
		...
		for (word_idx = 0; word_idx < PAGE_SIZE_INSTRS; word_idx = word_idx + 4) begin
				word = {memory[word_idx], memory[word_idx + 1], memory[word_idx + 2], memory[word_idx + 3]};
				flat_ctrl_bytes = flat_ctrl_bytes_start + word_idx;
				uart_to_mem_send_word(flat_ctrl_bytes, word);
		end
		uart_to_mem_send_word(24'h416000, 0); //write page_ctr to address 0x000 (address 0 in sram1) so the core can check if the next page is ready
		$display("Monitor: Done sending instruction page 0 [T=%0t]", $realtime);
		*/
		...
		while(word_ctr < MAX_BITBYTES/4) begin
			...
			if ((word_ctr + 1)% (PAGE_SIZE_BITSTR/4) == 0) begin
				$timeformat(-9, 2, " ns", 2);
				$display("Monitor: Done sending bitstream page %0d [T=%0t]", page_ctr, $realtime);
		...
		*/
		...
		flat_ctrl_bytes_start = 24'h416080; //swap endianness in word when sending
		while(word_ctr < NUM_INSTR) begin
			page_ctr = word_ctr / (PAGE_SIZE_INSTRS/4);
			page_idx = (page_ctr) % NUM_PAGES_INSTRS;
			flat_ctrl_bytes = flat_ctrl_bytes_start + (page_idx * PAGE_SIZE_INSTRS) + ((word_ctr % (PAGE_SIZE_INSTRS/4)) * 4);
			//$display("page_idx: %0d, flat_ctrl_bytes_start: %0h, (page_idx * PAGE_SIZE_BITSTR): %0d, ((word_ctr %% (PAGE_SIZE_BITSTR/4)) * 4): %0d, flat_ctrl_byes: %0d", page_idx, flat_ctrl_bytes_start, (page_idx * (PAGE_SIZE_BITSTR/4)), ((word_ctr % (PAGE_SIZE_BITSTR/4)) * 4), flat_ctrl_bytes);
			word = {memory[(word_ctr*4)], memory[(word_ctr*4) + 1], memory[(word_ctr*4) + 2], memory[(word_ctr*4) + 3]};

			uart_to_mem_send_word(flat_ctrl_bytes, word);

			if ((word_ctr + 1)% (PAGE_SIZE_INSTRS/4) == 0) begin
				$timeformat(-9, 2, " ns", 2);
				$display("Monitor: Done sending instruction page %0d [T=%0t]", page_ctr, $realtime);
				$display("page_idx: %0d, flat_ctrl_byes: %0h, word: %h", page_idx, flat_ctrl_bytes, word);
				uart_to_mem_send_word(24'h416000, page_ctr); //write page counter to address 0x000 (address 0 in sram2) so the core can check this if the next page is ready

				//TODO!!: check the page request and stall until page is requested, then continue with next line
				$display("TEST UART READ REQUEST [T=%0t]", $realtime);
				uart_to_mem_read_request(24'h42600c); //read request to address 00c
				#(8*BIT_PERIOD_UART_TO_MEM);
				$display("DONE SENDING UART READ REQUEST [T=%0t]", $realtime);
				$display("TEST UART READ REQUEST [T=%0t]", $realtime);
				uart_to_mem_read_request(24'h426080); //read request to address 080
				#(8*BIT_PERIOD_UART_TO_MEM);
				$display("DONE SENDING UART READ REQUEST [T=%0t]", $realtime);
			end
		...
		uart_to_mem_send_word(24'h416000, page_ctr);
		...
		*/
		...
		/*for (page_ctr = 0; page_ctr < (MEM_BYTES/4)/(PAGE_SIZE_INSTRS/4); page_ctr = page_ctr + 1) begin
			...
			uart_to_mem_send_word(24'h416000, page_ctr); //write page_ctr to address 0x000 (address 0 in sram1) so the core can check if the next page is ready
		end*/
	...
	//also read the word that is sent back 
	//not using output rx but global variable rxd_uart_to_mem to continuously assign value
	task uart_to_mem_read_request(input [23:0] ctrl);
		integer ctrl_byte_idx, bit_idx;
	...
	initial begin
		...
		wait(checkbits == 16'h0007); //after the first instruction page is written
	   	$display("Monitor: Start ibex (first instruction page was written) [T=%0t]", $realtime);
	...
	always @(i_counter) begin
		#1 $display("Counter: %b ", i_counter); //reminder: i_counter = mprj_io[37:32];
	end
	*/
	wire flash_csb;
	wire flash_clk;
